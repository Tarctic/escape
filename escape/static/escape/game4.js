...
const Main = ({ bgProp }) => {
    ...
    return (
        <div class="main">
            ...
            {obj.includes('door2') && <div className="door2" onClick={() => {if (obj.includes('key')) {panel2(),delFromInv('keyinv')} else {if (!speaking) {say("It's locked... Who would've guessed?")}}}}></div>}
            {obj.includes('clock') && <div className="clock" onClick={() => {if (!speaking) {say("The only thing faster than my math teacher...")}}}></div>}
            {obj.includes('db') && <div className="dustbin" onClick={() => {if (obj.includes('dbclick')) {addToInv('keyinv');delAddObj('dbclick',0); if (!speaking) {say("Huh? What's a key doing here?")}}}}></div>}
